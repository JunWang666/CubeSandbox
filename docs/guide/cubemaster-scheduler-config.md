...
- **Reservation behavior**: after selection, CubeMaster reserves the request's CPU/memory quota, one MVM slot, and one create-concurrency slot until the Cubelet create returns, so concurrent creates stop herding onto a node whose metrics have not caught up. Multi-replica deployments coordinate these reservations through Redis; `scheduler.reservation_redis_error_policy` controls what happens when that Redis write fails — `fail_open` (default: local-only accounting, matching pre-reservation behavior) or `fail_closed` (fail the scheduling attempt, recommended for multi-replica deployments). See [Extensible scheduler plugins](./scheduler-plugin.md) for the full semantics.
