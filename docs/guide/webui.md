...
- **Auth modes** — CubeAPI supports two auth modes (plus fully open), and its unified auth middleware protects every route, the terminal included. Because browsers cannot set headers on a WebSocket handshake, the terminal endpoint validates the credential itself, mirroring the middleware's logic. See [Authentication](./authentication.md).
  ...
  - **Auth disabled (open mode)**: the terminal **fails closed** — with no auth backend configured, terminal handshakes are rejected with `403` by default, because an open terminal is a root shell into every sandbox. Set `TERMINAL_ALLOW_UNAUTHENTICATED=true` to explicitly enable it for development; anyone who can then reach the Dashboard can open a terminal into any running sandbox, so treat Dashboard access accordingly. (One-click/Helm do not run open: they wire the CubeOps verifier above. To run them without auth, set `AUTH_CALLBACK_URL=off` / `authCallbackUrl: "off"`.)
