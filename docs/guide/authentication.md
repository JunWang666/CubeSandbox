...
**Built-in verifier:** CubeOps ships a ready-made callback at `POST /api/v1/auth/verify` that validates the WebUI login JWT and returns the verified username in `X-Auth-User`. The bundled deployments wire it automatically — one-click exports `AUTH_CALLBACK_URL=http://127.0.0.1:3010/api/v1/auth/verify` (override with your own URL, or `off` to disable auth), and the Helm chart defaults `controlPlane.api.authCallbackUrl` to `"auto"` (pointing at the CubeOps Service; `"off"` disables auth). With no auth backend configured, the terminal endpoint fails closed unless `TERMINAL_ALLOW_UNAUTHENTICATED=true`.
