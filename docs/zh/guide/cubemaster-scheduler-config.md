...
- **预留行为**：选定节点后，以进程内锁检查并预留 CPU/内存、MVM 和创建并发槽位，Cubelet 返回后释放。预留不访问 Redis，也不提供跨 Master 原子容量协调。旧的 `scheduler.reservation_redis_error_policy` 已移除。完整语义与限制见[可扩展调度插件](./scheduler-plugin.md)。
